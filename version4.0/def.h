...
struct ASTNode {
	...
	union {
          ...
          char  type_char;              //由字符常值生成的叶结点
          char  type_string[50];            //由字符串值生成的叶结点    ?????????????????????????????
	      };
...
struct symbol {       //这里只列出了一个符号表项的部分属性，没考虑属性间的互斥
    ...
    int  paramnum;  //对函数适用，记录形式参数个数  个人理解为对数组也适用，数组的每个值就相当于
    int  dimension[2]; //对数组适用，记录数组维的大小或者索引 int数组的默认值是0，代表没有这个维
    int  dimensionNum; //对数组适用，记录数组维的数目
    char alias[10];   //别名，为解决嵌套层次使用
    char flag;       //符号标记，函数：'F'  变量：'V'   参数：'P'  临时变量：'T'  常量： 'C' 数组：'A' 数组项：'I'
...
//符号表
struct symboltable{
    struct symbol symbols[MAXLENGTH];//符号数组
    int index;//符号位置索引
    } symbolTable;


//新作用域的起点位置：当前作用域的符号在符号表的起始位置序号,栈结构,当使用顺序表作为符号表时，进入、退出一个作用域时需要对其操作，以完成符号表的管理。对其它形式的符号表，不一定需要此数据结构
struct symbol_scope_begin {
    int TX[30];//将符号表symbolTable的栈顶位置symbolTable.index保存在栈  一般根据栈顶来遍历符号表
    int top;
    } symbol_scope_TX;
