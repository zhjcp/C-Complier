...
//  %type 定义非终结符的语义值类型
%type  <ptr> program ExtDefList ExtDef  Specifier ConstSpecifier ExtDecList FuncDec CompSt VarList VarDec  ParamDec Stmt StmList SKIP DefList Def DecList Dec Exp Args /*删除，防止编译错误！！：CaseStmtList0 CaseStmtList*/
...
%token PLUS SELFPLUS MINUS SELFMINUS STAR DIV ASSIGNOP PLUS_ASSIGNOP MINUS_ASSIGNOP STAR_ASSIGNOP DIV_ASSIGNOP AND OR NOT IF ELSE WHILE RETURN FOR SWITCH CASE  DEFAULT 
/*以下为接在上述token后依次编码的枚举常量，作为AST结点类型标记*/
%token EXT_DEF_LIST EXT_VAR_DEF EXT_VARCONST_DEF FUNC_DEF FUNC_DEC EXT_DEC_LIST PARAM_LIST PARAM_DEC VAR_DEF DEC_LIST DEF_LIST COMP_STM STM_LIST EXP_STMT SKIP_SEMI IF_THEN IF_THEN_ELSE CASE_THEN DEFAULT_THEN
...
program: ExtDefList    { display($1,0); }//显示语法树,语义分析   //semantic_Analysis0($1);暂时没有定义，先删掉，防止编译报错！！！    
         ; 
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(2,EXT_VAR_DEF,yylineno,$1,$2);}   //该结点对应一个外部变量声明
         ...
         |ConstSpecifier  Exp  SEMI {$$=mknode(2,EXT_VARCONST_DEF,yylineno,$1,$2);}   //该结点对应一个常量定义 后面可以通过定义一个Exp的继承属性，告诉这个Exp赋值表达式它是常量
         | error SEMI   {$$=NULL;}
         ;
...
VarDec:   ID          {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
        | VarDec LB INT RB     {strcpy($$->type_id,$1->type_id);} /*多维数组的标识符,采用递归，但是左递归可能会导致一些意外  现在还没有提取数组的大小，只是提取了数组的名字   在ID归约时就建立了语法树结点，在这里不再需要建立树结点*/
         ;
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | ID            {$$=mknode(0,ID,yylineno); strcpy($$->type_id,$1);} /* type_id是ID的语义值，比如：a=1，a的语义值就是1 这里还传给Exp让其之后使用*/
